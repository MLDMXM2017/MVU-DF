'''
Description: 多模态不确定深度森林
             sklearn V.0.22 documentation: https://scikit-learn.org/0.22
             
Author: tanqiong
Date: 2023-04-18 17:09:34
LastEditTime: 2023-09-01 14:28:46
...
from .util import (record_opinion_sorted_by_uncertainty, record_opinion_res, df_to_csv, 
                    get_stage_matrix, get_scores, metrics_name, generate_corr_pic)
from .dataloader import DataLoader
from .evaluation import accuracy, f1_macro, mse_loss, aupr, auroc
from .beam_search import inter_operator_fit, inter_operator_transform
from hiDF.intersection import (inter_operator, intra_generation, feature_intersection, 
                                remove_highly_correlated_features)
from hiDF.intersection import select_besk_k
...
class MVUGCForest(object):
    ...
    def __init__(self, config: dict):
        ...
        self.boost_feature_type: str = config.get("boost_feature_type", 'opinion')
        self.replace_belief: bool = config.get("replace_belief", False)                     # 是否使用proba替换opinion中的belief
        
        # high order features
        self.is_RIT = config.get("is_RIT", True)                                            # 高阶特征 交互开关
        self.is_intersection_inner_view = config.get("is_intersection_inner_view", True)   # 视图内 特征交互开关
        self.is_intersection_cross_views = config.get("is_intersection_cross_views", True) # 视图间 特征交互开关
        self.inter_method = config.get("inter_method", "stack")                             # 视图间 特征交互的方法，'stack' or 'operator'
        self.enforce_intra = config.get("enforce_intra", False)                             # 是否强迫生成intra-section feature
        self.intra_feature_least = config.get("intra_feature_least", 1)                     # 强迫生成intra-section的数量
        self.filter_intra = config.get("filter_intra", False)                               # 过滤开关
        self.filter_inter = config.get("filter_inter", False)                               # 过滤开关
        ...
        self.uncertainty_basis = config.get("uncertainty_basis", "evidence")
        self.act_func = config.get("act_func", "approx_step")
        self.W_type = config.get("W_type", "sum") # 'n_class', 'n_tree', 'sum', 'variable'
        self.use_kde = config.get("use_kde", False) # 使用kde增加数据不确定度
        ...
        if config.get("bypass_prediction", None) is not None:
            self.bypass_prediction = config.get("bypass_prediction", None)
        ...
        for item in self.estimator_configs:        # Dynamic Ensemble Selection
            item["is_des"] = self.is_des
            item["evidence_type"] = self.evidence_type
            item["uncertainty_basis"] = self.uncertainty_basis
            item["act_func"] = self.act_func
            item["W_type"] = self.W_type
            item["use_kde"] = self.use_kde
    ...
    def fit_multiviews(self, x_train: List[arr], y_train: arr, 
        ...
        """
        # Parameters
        cluster_samples_view = True
        cluster_samples_layer = True
        ...
        while depth < self.max_layers:
            ...
            # 如果样本加权
            if self.is_weight_bootstrap and uncertainty_origin is not None:
                sample_weight[:n_origin_sample] += uncertainty_origin

            for v in range(self.n_view):
                print(f"  train view {v} ")
                self.LOGGER_MULTIVIEW_T.info(f"  -----------view-{v}-----------")
                
                n_nodes = len(self.estimator_configs)
                n_train_origin_sample = np.sum(train_origin_mask)
                
                # 获取view对应的训练数据
                x_train_dict[v] = dl_train.features[v]
                y_train = dl_train.labels[v]
                n_samples_train = len(x_train_dict[v])
                ...
                # print(f"    output shape, proba list: {np.shape(proba_list)}, opinion list: {np.shape(opinion_list), }")
                ...
                if self.uncertainty_basis == "evidence":
                    evidence_train_origin_list = [evidence[:n_train_origin_sample] for evidence in evidence_list]

                # 单个视图内的特征交互
                if self.is_RIT:
                    
                    # 利用RIT生成交互特征, 传入训练的训练数据的交互特征
                    print('    Start bootstrap, RIT...')
                    new_features_lst, new_feature_stability, bootstrap_feature_importance = \
                        feature_intersection(
                            layer,
                            X_train = x_train_dict[v], 
                            y_train=y_train,
                            X_test = None, 
                            y_test = None,
                            use_RIT = True,
                            ...
                            noisy_split=False,
                            num_splits=2,
                            enforce_intra=self.enforce_intra,                               # 是否强迫生成intra-section feature
                            new_feature_least=self.intra_feature_least,                         # 强迫生成intra-section的数量
                        )

                    self.new_features_lst[f'layer_{depth}_view_{v}'] = new_features_lst
                    self.new_feature_stability[f'layer_{depth}_view_{v}'] = new_feature_stability
                    self.bootstrap_feature_importance[f'layer_{depth}_view_{v}'] = bootstrap_feature_importance
                    # print('    Finish bootstrap, RIT...')

                # 是否使用不确定性作为增强特征, 传入训练的训练数据的交互特征
                if self.boost_feature_type == 'opinion':
                    boost_features = np.hstack(opinion_list)                  
                    # 把opinion中的belief使用proba替换
                    if self.replace_belief:
                        for k, proba in enumerate(proba_list):
                            boost_features[:, k*(n_class+1):(k+1)*(n_class+1)-1] = proba
                    
                    
                elif self.boost_feature_type == 'probability':
                    boost_features = np.hstack(proba_list)
                elif self.boost_feature_type == False:
                    boost_features = np.empty((n_samples_train, 0))
                else:
                ...
                if boost_features is not None: # 增强特征
                    ...
                    # 记录相关性
                    self.record_boost_features_corr(depth, v, boost_features, y_train, "train")
                # intra-交互特征
                if self.is_intersection_inner_view:   
                    intra_features = intra_generation(
                        x_train_dict[v], 
                        new_features_lst,
                    ...
                    # 记录相关性
                    self.record_intra_features_corr(depth, v, intra_features, y_train, "train")
                    # 高阶特征筛选
                    if self.filter_intra:
                        intra_features, idx = select_besk_k(intra_features, y=y_train, k=10)
                        self.intra_selected_idx[f"layer_{depth}_view_{v}"] = idx
                    intra_dict[v] = intra_features
                    print(f"    generate intra-view feature number:\t{intra_features.shape[1]}")
                    # 使用intra_features更新x_train 
                    x_train_dict[v] = dl_train.update_intersection_features(
                        v, 
                        depth,
                        intra_features, 
                        is_stacking=True,
                ...
                # 计算单个view的evidence, 基于原始样本
                if self.uncertainty_basis == "evidence":
                    _evidence_origin_view_i = np.mean(evidence_train_origin_list, axis=0)
                    if depth == 0:
                        evidence_origin_view_i = _evidence_origin_view_i
                    else:
                        evidence_origin_view_i = deepcopy(views_evidence_origin_dict[v][-1])
                        evidence_origin_view_i[train_origin_mask] = _evidence_origin_view_i
                    views_evidence_origin_dict[v].append(evidence_origin_view_i)

                # 日志记录 每个view的opinion和精确度, base: 传入训练的原始样本
                self.OPINION_LOGGER_T.info(
                    f"  view opinion,\tview: {v}, \n{opinion_origin_view_i}")
                if "loss" in evaluate_name:
                    score_view_origin_sample = evaluate(y_origin, np.log(evidence_origin_view_i), depth+1)
                elif "opinion" in evaluate_name:
                    score_view_origin_sample = evaluate(y_origin, opinion_origin_view_i)
                else:
                ...
                # 0609: 视图内采样几乎已被弃用, 不再对其维护
                # 视图内的样本聚簇, 视图内的训练数据更新
                # if classify_samples and (depth+1>=self.span):
                if cluster_samples_view:
                    # 
                    # sample_marks_view = self.group_samples_base_span(depth, views_opinions_origin_dict[v][-self.span:], y_origin)
                    sample_marks_view = self.cluster_sample(self.cluster_method, 
                                                                depth, 
                                                                views_opinions_origin_dict[v],
                                                                y_origin,)
                    
                    if np.sum(sample_marks_view==2)>0: 
                        self.LOGGER_MULTIVIEW_T.info(f"  number of easy sample\t{np.sum(sample_marks_view==2)}")
                        self.LOGGER_MULTIVIEW_T.info(f"  number of hard sample\t{np.sum(sample_marks_view==3)}")
                        self.LOGGER_MULTIVIEW_T.info(f"  number of outlier sample\t{np.sum(sample_marks_view==4)}")
                        if "loss" in evaluate_name:
                            score_view_easy_sample = evaluate(y_origin[sample_marks_view==2], np.log(evidence_origin_view_i[sample_marks_view==2]), depth+1)
                        elif "opinion" in evaluate_name:
                            score_view_easy_sample = evaluate(y_origin[sample_marks_view==2], opinion_origin_view_i[sample_marks_view==2])
                        # elif "au" in evaluate_name:
                        #     score_view_easy_sample = evaluate(y_origin[sample_marks_view==2], opinion_origin_view_i[sample_marks_view==2])
                        else:
                            score_view_easy_sample = evaluate(y_origin[sample_marks_view==2], opinion_to_proba(opinion_origin_view_i[sample_marks_view==2]))
                        self.LOGGER_MULTIVIEW_T.info(f"  The evaluation[{evaluate_name}] of easy sample is {score_view_easy_sample:.4f}")
            ...
            # 多个views之间的交互
            if self.is_intersection_cross_views:
                inter_features = self.inter_generation(
                    x_train_dict, depth, intra_dict=intra_dict, 
                    stage="train", labels=y_train,
                )
                
                # 高阶特征选择
                if self.filter_inter:
                    inter_features, idx = select_besk_k(inter_features, y=y_train, k=10)
                    self.inter_selected_idx[f"layer_{depth}"] = idx
                
                # 更新特征
                for v in range(self.n_view):
                    dl_train.update_intersection_features(v, 
                                                          depth,
                                                          inter_features, 
                                                          is_stacking=True,
                                                          ftype="inter")
                
                self.record_inter_features_corr(depth, inter_features, y_train, 'train')
            ...
            if self.is_weight_bootstrap:
                uncertainty_origin = joint_opinion_cur_layer_origin[:, -1]
            ...
            # 层采样
            if cluster_samples_layer and sample_marks_layer is not None:
                if self.is_resample:
                    ...
                    elif self.layer_resample_method == "respective":
                        x_new_dict, y_new, n_from_hard, n_from_outlier = dl_train.resample_layer_for_views(
                            depth,
                            sample_marks_layer,
                            fold=( 1+( 1/(depth+1) ) )
                        )
                    ...
                    if len(y_new) != 0:
                        ...
                        for li in range(depth+1):
                            ...
                            for vi in range(self.n_view):
                                ...
                                # 进行视图内特征交互
                                if self.is_intersection_inner_view:
                                    intra_features_new = intra_generation(
                                        x_new_vi, 
                                        self.new_features_lst[f"layer_{li}_view_{vi}"],
                                        self.new_feature_stability[f"layer_{li}_view_{vi}"],
                                        self.bootstrap_feature_importance[f"layer_{li}_view_{vi}"],
                                    )
                                    if self.filter_intra:
                                        intra_features_new, _ = select_besk_k(intra_features_new, support=self.intra_selected_idx[f"layer_{li}_view_{vi}"])
                                    intra_new_dict[vi] = intra_features_new
                                    x_new_vi = np.hstack([x_new_vi, intra_features_new])

                                # 为第v个view的新样本构造完增强特征之后更新该view的新样本
                                x_new_dict[vi] = x_new_vi
                            
                            # 为新样本构建视图间的交互特征:
                            # 每个view生成完特征boost_feature和inner_intersection后, 对每个view统一加上 cross_intersection
                            if self.is_intersection_cross_views:
                                inter_features_new = self.inter_generation(x_new_dict, li, intra_new_dict, stage="predict")
                                
                                if self.filter_inter:
                                    inter_features_new, _ = select_besk_k(inter_features_new, support=self.inter_selected_idx[f"layer_{li}"], k=10)
                                    
                                # 为每个view的新样本添加交互特征, 默认是直接加在最后的列
                                for vi in range(self.n_view):
                                    x_new_dict[vi] = np.hstack([x_new_dict[vi], inter_features_new])
            ...
            # 固定 train_origin_mask, 过滤掉易分样本不进入下一层的训练
            filter_easy_fitting = False
            if filter_easy_fitting and sample_marks_layer is not None:  
                train_origin_mask[sample_marks_layer==2] = False 
            ...
            if "loss" in evaluate_name:
                # 基于证据的评分
                evidence_layer = np.log(np.sum([view_evidences[-1] for view_evidences in views_evidence_origin_dict.values()], axis=0))
                current_evaluation = evaluate(y_origin, evidence_layer, depth+1)
            elif "opinion" in evaluate_name:
                current_evaluation = evaluate(y_origin, joint_opinion_cur_layer_origin)
            else:
            ...
            if (sample_marks_layer is not None) and evaluate_set == "trouble":
                ...
                if "loss" in evaluate_name:
                    cur_evaluation_not_easy = evaluate(y_origin[mask_eval], evidence_layer[mask_eval], depth+1)
                elif "opinion" in evaluate_name:
                    cur_evaluation_not_easy = evaluate(y_origin[mask_eval], joint_opinion_cur_layer_origin[mask_eval])
                else:
        ...
        for v, view in enumerate(views.values()):
            self.LOGGER_MULTIVIEW_T.info(
                f"  view {v}:\t{[node.estimator_type for node in view.layers[0].nodes]}")
    ...
    def predict_opinion(self, 
        ...
        for depth in range(n_layer):
            ...
            if depth < n_layer-1:
                ...
                for v in range(self.n_view):
                    ...
                    if self.is_intersection_inner_view: 
                        intra_features = intra_generation(
                            x_test_dict[v], 
                            self.new_features_lst[f"layer_{depth}_view_{v}"],
                            self.new_feature_stability[f"layer_{depth}_view_{v}"],
                            self.bootstrap_feature_importance[f"layer_{depth}_view_{v}"],
                        )
                        # 记录相关性
                        self.record_intra_features_corr(depth, v, intra_features, y_test, "predict")
                        if self.filter_intra:
                            intra_features, _ = select_besk_k(intra_features, support=self.intra_selected_idx[f"layer_{depth}_view_{v}"])
                        # 更新交互特征
                        intra_dict[v] = intra_features
                        x_test_dict[v] = np.hstack([x_test_dict[v], intra_features]) 
                    ...
                    if evidence_list is not None:
                        evidence_view_i = np.mean(evidence_list, axis=0)
                    view_evidence_list.append(evidence_view_i)
                    
                    # 记录LOG
                    if "loss" in evaluate_name:
                        self.LOGGER_MULTIVIEW_V.info("  The evaluation[{}] of view_{} is {:.4f}".format(
                                evaluate_name, v, evaluate(y_test, np.log(evidence_view_i), depth+1)))
                    elif "opinion" in evaluate_name:
                        score_view_origin_sample = evaluate(y_test, opinion_view_i)
                    else:
                        self.LOGGER_MULTIVIEW_V.info("  The evaluation[{}] of view_{} is {:.4f}".format(
            ...
            # 中间层
                ...
                for v in range(self.n_view):
                    if (np.any(np.isnan(x_test_dict[v]))):
                        print(v)
                    # 预测
                    proba_list, opinion_view_i, evidence_view_i = self.views[v].predict_use_layer(
                        x_test_dict[v], depth, self.view_opinion_generation_method)
                    view_opinion_list.append(opinion_view_i)
                    view_evidence_list.append(evidence_view_i)
                    ...
                    if "loss" in evaluate_name:
                        ...
                        self.LOGGER_MULTIVIEW_V.info("The evaluation[{}] of view_{} is {:.4f}".format(
                            evaluate_name, v, evaluate(y_test, opinion_view_i)))
                        # score_view_origin_sample = evaluate(y_origin, joint_opinion_cur_layer_origin)
                    else:
                        self.LOGGER_MULTIVIEW_V.info("The evaluation[{}] of view_{} is {:.4f}".format(
                            evaluate_name, v, evaluate(y_test, opinion_to_proba(opinion_view_i))))
            ...
            # 多视图之间交互
            if self.is_intersection_cross_views and depth < n_layer-1:
                inter_features = self.inter_generation(x_test_dict, depth, intra_dict, stage="predict")
                if self.filter_inter:
                    inter_features, _ = select_besk_k(inter_features, support=self.inter_selected_idx[f"layer_{depth}"], k=10)
                # 更新交互特征
                for v in range(self.n_view):
                    x_test_dict[v] = np.hstack([x_test_dict[v], inter_features])
            
                # 如果y_test不是None
                if y_test is not None:  
                    self.record_inter_features_corr(depth, inter_features, y_test, "predict")
            ...
            # 本层的预测概率
            proba_layer = opinion_to_proba(opinion_layer)

            # 记录一层的evidence
            if view_evidence_list[0] is not None:
                evidence_layer = np.mean(view_evidence_list, axis=0)
                evidence_layer = evidence_layer[:, :self.n_class]
            else:
                evidence_layer = None
            ...
            if bypass:
                activate_mask = ~fixed_mask
                u_lower_mask = opinion_layer[:, -1]<predict_opinion[:, -1] # 标记出不确定度下降的样本
                update_mask = activate_mask | u_lower_mask
            else:
                update_mask = np.ones(n_origin_sample, dtype=bool)  # 全部都要更新

            predict_opinion[update_mask] = opinion_layer[update_mask]
            predict_proba[update_mask] = proba_layer[update_mask]
            if evidence_layer is not None:
                predict_evidence[update_mask] = evidence_layer[update_mask]
            
            layers_opinions_list.append(predict_opinion.copy())
           
            # 更新样本的标记
            if self.cluster_method:
                sample_marks_layer_cur_layer = self.cluster_sample(self.cluster_method, depth, layers_opinions_list)
                
                # 更新fixed_mask
                new_fixed_mask = sample_marks_layer_cur_layer == 2       # 新一层要被固定结果的掩码矩阵
                fixed_mask = fixed_mask | new_fixed_mask                 # 或运算

                # 记录难分样本易分样本日志
                self.LOGGER_MULTIVIEW_V.info(f"number of normal sample\t{np.sum(sample_marks_layer_cur_layer==1)}")
                self.LOGGER_MULTIVIEW_V.info(f"number of easy sample\t{np.sum(sample_marks_layer_cur_layer==2)}")
                self.LOGGER_MULTIVIEW_V.info(f"number of hard sample\t{np.sum(sample_marks_layer_cur_layer==3)}")
                self.LOGGER_MULTIVIEW_V.info(f"number of outlier sample\t{np.sum(sample_marks_layer_cur_layer==4)}")
                mark_dict = {1:'normal sample', 2:'easy sample', 3:'hard sample', 4:'outlier sample'}
                for m in range(1, 5):
                    mean_u = np.mean(opinion_layer[sample_marks_layer_cur_layer==m, -1])
                    std_u = np.std(opinion_layer[sample_marks_layer_cur_layer==m, -1])
                    self.LOGGER_MULTIVIEW_V.info(f"{mark_dict[m]}: mean uncertainty {mean_u:.4f}\t std uncertainty {std_u:.4f}")

                # 记录测试集的中间结果
                mkdir(self.logger_path+"/sample_nn_predict")
                mkdir(self.logger_path + "/predict_results")
                for v, x in enumerate(x_test_dict.values()):
                    x = x[:, :n_origin_feature_list[v]]
                    predict_result_record = record_opinion_res(y_test, opinion_layer, sample_marks_layer_cur_layer)
                    df_to_csv(predict_result_record, self.logger_path + "/predict_results", f"result_layer_{depth}_view_{v}.csv")

            sample_marks_layer_df[f"layer_{depth}"] = sample_marks_layer_cur_layer
            ...
            # 易分样本的数量
            self.LOGGER_MULTIVIEW_V.info(
                f"new easy sample number:\t{np.sum(fixed_mask)-np.sum(new_fixed_mask)}"
            )
            self.LOGGER_MULTIVIEW_V.info(
                f"total easy sample number:\t{np.sum(fixed_mask)} "
            )
            # 被判定为易分样本的识别准确度
            if np.sum(fixed_mask) != 0:
                if "loss" in evaluate_name:
                    self.LOGGER_MULTIVIEW_V.info(f"The evaluation[{evaluate_name}] of easy sample:\t{evaluate(y_test[fixed_mask], predict_proba[fixed_mask], depth+1):.4f}" )
                elif "opinion" in evaluate_name:
                    self.LOGGER_MULTIVIEW_V.info(f"The evaluation[{evaluate_name}] of easy sample:\t{evaluate(y_test[fixed_mask], predict_opinion[fixed_mask]):.4f}" )
                else:
                    self.LOGGER_MULTIVIEW_V.info(f"The evaluation[{evaluate_name}] of easy sample:\t{evaluate(y_test[fixed_mask], predict_proba[fixed_mask]):.4f}" )

            # 所有样本的识别准确度
            if "loss" in evaluate_name:
                self.LOGGER_MULTIVIEW_V.info("The evaluation[{}] of layer {} with fixed is {:.4f}".format(
                    evaluate_name, depth, evaluate(y_test, predict_evidence, depth+1)))
                self.LOGGER_MULTIVIEW_V.info("The evaluation[{}] of layer {} without fixed is {:.4f}".format(
                    evaluate_name, depth, evaluate(y_test, evidence_layer, depth+1)))
            elif "opinion" in evaluate_name:
                self.LOGGER_MULTIVIEW_V.info("The evaluation[{}] of layer {} with fixed is {:.4f}".format(
                    evaluate_name, depth, evaluate(y_test, predict_opinion)))
                self.LOGGER_MULTIVIEW_V.info("The evaluation[{}] of layer {} without fixed is {:.4f}".format(
                    evaluate_name, depth, evaluate(y_test, opinion_layer)))
            else:
                self.LOGGER_MULTIVIEW_V.info("The evaluation[{}] of layer {} with fixed is {:.4f}".format(
        ...
        if self.boost_feature_type == 'opinion':
            ...
            if self.replace_belief:
                for k in range(n_node):
                    boost_features[ :, k*(n_class+1):(k+1)*(n_class+1)-1 ] = \
                        proba[ :, k*n_class:(k+1)*n_class ]
    ...
    def inter_generation(self, x_dict:dict, depth:int, intra_dict=None, stage:str=None, labels:np.ndarray=None):
        """模态之间的特征交互
        Parameters
        ----------
        x_dict: dict, 
        depth: int, 深度森林当前的深度
        intra_dict:
        stage: str, 'predict' or 'train'
        labels: ndarray, 标签数据, 仅在stage='train'时启用

        Returns
        -------
        x_intersection: ndarray of shape(#samples, #new features)
        """
        method = self.inter_method    #"stack", "operator"
        x_intersection = None
        if method == "stack":
            for v in range(self.n_view):
                intersection_features = intra_generation(
                    x_dict[v], 
                    self.new_features_lst[f"layer_{depth}_view_{v}"],
                    self.new_feature_stability[f"layer_{depth}_view_{v}"],
                    self.bootstrap_feature_importance[f"layer_{depth}_view_{v}"],
                ...
                if x_intersection is None:
                    x_intersection = intersection_features
                else:
                    x_intersection = np.hstack([x_intersection, intersection_features])
        elif method == "operator":
            x_intersection = self.inter_operator_generation(
                intra_dict, labels, depth, stage,
            )
                
        return x_intersection

    def inter_operator_generation(self, intra_dict, labels, depth, stage):
        """基于运算符的模态间交互特征生成"""
        assert self.inter_method == "operator", "the para operator must be 'operator'!"
        assert intra_dict is not None, "the para intra_dict should not be None!"
        is_beam_search = True   # 是否使用束搜索
        if is_beam_search:
            if stage == "train":
                random_state = self.random_state+self.random_state*depth if self.random_state is not None else None
                x_intersection, operations, inter_orders = inter_operator_fit(
                    intra_dict=intra_dict, label=labels, random_state=random_state
                )
                self.inter_operations_dict[f"layer_{depth}"] = operations
                self.inter_orders_dict[f"layer_{depth}"] = inter_orders
                print(f"inter_list feature shape in layer {depth}: ", np.shape(x_intersection))
            elif stage == "predict":
                x_intersection = inter_operator_transform(
                    intra_dict=intra_dict, 
                    operations=self.inter_operations_dict[f"layer_{depth}"],
                    inter_orders= self.inter_orders_dict[f"layer_{depth}"],
                )
            pass
        else:
            if stage == "train":
                random_state = self.random_state+self.random_state*depth if self.random_state is not None else None
                x_intersection, operations = inter_operator(
                    intra_dict=intra_dict, 
                    random_state=random_state,
                )
                self.inter_operations_dict[f"layer_{depth}"] = operations
            elif stage == "predict":
                x_intersection, _ = inter_operator(
                    intra_dict=intra_dict, 
                    operations=self.inter_operations_dict[f"layer_{depth}"]
                )
        ...
        # 筛选特征之间相关性较高的特征
        if stage == "train":
            x_intersection, remove_idx = remove_highly_correlated_features(x_intersection, threshold=0.7)
            self.inter_remove_idx_dict[f"layer_{depth}"] = remove_idx
        elif stage == "predict":
            x_intersection, remove_idx = remove_highly_correlated_features(
                x_intersection, remove_idx=self.inter_remove_idx_dict[f"layer_{depth}"] ) 
        else:
            raise ValueError("Para stage should be 'train' or 'predict' ")
        return x_intersection
    ...
    def generate_view_opinion(self, opinion_list, view_opinion_generation_method):
        """生成 view的综合opinion"""
        if view_opinion_generation_method == "joint":   # 对node产出的opinion进行joint
            opinion_view = joint_multi_opinion(opinion_list)
        elif view_opinion_generation_method == "mean":  # 平均node产出的opinion
            opinion_view = np.mean(opinion_list, axis=0)
        elif view_opinion_generation_method == "sum":   # 对node产出的opinion进行加和, 然后再归一化, 这tm跟mean一样啊
            opinion_view = np.sum(opinion_list, axis=0)
            opinion_view /= np.sum(opinion_view, axis = 1, keepdims=True)
        else:
    ...
    def cluster_sample(self, cluster_method:str, depth:int, opinions_list:List, y=None):
        ...
        elif cluster_method == 'uncertainty':
            sample_marks_layer = self.group_samples_by_uncertainty(depth, opinions_list[-1], y)
        elif cluster_method == 'uncertainty_bins':
            sample_marks_layer = self.group_samples_by_uncertainty_bins(depth, opinions_list[-1], y)
        else:
